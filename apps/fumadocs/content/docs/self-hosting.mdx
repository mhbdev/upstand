...
The installer also records `TRUSTED_PROXY_CIDRS` from the configured Swarm
overlay network. The API accepts `X-Forwarded-For` and `X-Real-IP` only when
the immediate peer is inside one of these CIDRs. If you run a custom proxy
topology, set this variable to the proxy network CIDRs and never include
client-facing networks.
