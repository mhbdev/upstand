...
The installer writes the generated credentials and resolved image references to `/etc/upstand/.env` with mode `0600`. Treat that file as a secret. It also uses `docker stack deploy --resolve-image always --with-registry-auth --prune` so a rollout has a resolved image digest and stale services are removed.
