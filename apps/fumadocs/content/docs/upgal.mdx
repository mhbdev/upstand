...
Organization owners and administrators configure UpGal from **Settings → AI**.

1. Choose OpenAI, Anthropic, Google, or the OpenAI-compatible AI Gateway.
2. Enter the model identifier supported by that provider.
3. Enter the API key and, when required, a custom base URL.
4. Save the settings and use **Test connection** to verify the configuration.
...
UpGal exposes a reusable JSON-RPC MCP endpoint at `/api/mcp`. Create an external key from **Settings → AI**, choose only the tool scopes required by the integration, and keep the returned secret in a password manager. The secret is shown once; the database stores only its SHA-256 hash.
