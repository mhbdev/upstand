...
The Templates page also provides an AI draft generator. It returns validated
Compose YAML only; it does not save a template or deploy a resource. Review
the generated editor contents, then use the existing 2FA-protected save and
explicit deployment actions. Generated drafts reject privileged mode, host
namespaces, host bind mounts, Docker socket mounts, capabilities, and devices.
