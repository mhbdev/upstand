...
![Upstand Dashboard Overview](/images/dashboard_hero.svg)
...
flowchart TD
    ...
    UpGal -->|Tool Loop| Server
    Server --> DB[(PostgreSQL System DB)]
    Server --> Queue[(Redis / BullMQ Queue)]
    Queue --> Worker[Deployment Worker Queue]
    Worker -->|Docker Socket / SSH| Engine[Docker Swarm Engine]
    Worker -->|Atomic Reloads| Caddy[Caddy Proxy]
    Engine -->|Overlay Network| Containers[App & DB Containers]"
/>
