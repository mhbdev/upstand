...
async function runCLI(command: string, arg?: string): Promise<void> {
  const log: string[] = [];
  ...
  switch (command.toLowerCase()) {
    ...
    default:
      ...
      console.error(
        "Usage: bun cli [create|remove|check-types|db:generate|db:push|format] [name]",
      );
      process.exit(1);
  }
}
...
const showMainMenu = () => {
  ...
  const options = [
    ...
    {
      name: "✦ Add Procedure",
      description: "Add a procedure to existing module",
      value: "add-proc",
    },
    {
      name: "✕ Remove Module",
...
// ═══════════════════════════════════════════════════════════════════════════════
// ADD PROCEDURE — multi-step wizard
// ═══════════════════════════════════════════════════════════════════════════════

const showAddProcedureWizard = () => {
  const modules = gen.getActiveModules();
  if (!modules.length) {
    clearLog("No generated modules found. Create a module first.\n");
    showMainMenu();
    return;
  }

  const state: ProcedureConfig = {
    moduleName: "",
    procedureName: "",
    procedureKind: "query",
    procedureAccess: "public",
    useCaseToken: undefined,
    inputSchema: undefined,
  };

  showAddProcStep1(state, modules);
};

// Step 1 ── Select module
const showAddProcStep1 = (state: ProcedureConfig, modules: string[]) => {
  clearLeft();
  setLeftTitle("Add Procedure · Module");
  clearLog("Select the module to add a procedure to.\n");

  const opts = [
    ...modules.map((m) => ({
      name: m.charAt(0).toUpperCase() + m.slice(1),
      description: `Add to ${m}Router`,
      value: m,
    })),
  ...
  sel.on(
    ...
    (_idx: number, opt: { value: string }) => {
      ...
      showAddProcStep2(state);
    },
  );
};

// Step 2 ── Procedure name
const showAddProcStep2 = (state: ProcedureConfig) => {
  clearLeft();
  setLeftTitle("Add Procedure · Name");
  clearLog(
    `Adding procedure to: ${state.moduleName}Router\n\nExamples: list, getById, publish, archive\n`,
  );
  ...
  const cleanup = onEscape(() =>
    showAddProcStep1(state, gen.getActiveModules()),
  );

  input.on(InputRenderableEvents.ENTER, (val: string) => {
    cleanup();
    const name = val.trim().replace(/[^a-zA-Z0-9_]/g, "");
    if (!name) {
      appendLog("[!] Procedure name cannot be empty.\n");
      return;
    }
    state.procedureName = name;
    showAddProcStep3(state);
  });
};

// Step 3 ── Query or mutation
const showAddProcStep3 = (state: ProcedureConfig) => {
  clearLeft();
  setLeftTitle("Add Procedure · Kind");
  ...
  const cleanup = onEscape(() => showAddProcStep2(state));
  ...
  sel.on(
    ...
    (_idx: number, opt: { value: string }) => {
      ...
      showAddProcStep4(state);
    },
  );
};

// Step 4 ── Access level
const showAddProcStep4 = (state: ProcedureConfig) => {
  clearLeft();
  setLeftTitle("Add Procedure · Access");
  ...
  const cleanup = onEscape(() => showAddProcStep3(state));
  ...
  sel.on(
    ...
    (_idx: number, opt: { value: string }) => {
      cleanup();
      state.procedureAccess = opt.value as "public" | "protected";
      showAddProcStep5(state);
    },
  );
};

// Step 5 ── Wire use-case (optional)
const showAddProcStep5 = (state: ProcedureConfig) => {
  clearLeft();
  setLeftTitle("Add Procedure · Use Case");
  clearLog(
    `Procedure: ${state.procedureName} (${state.procedureKind})\nModule: ${state.moduleName}\n\nOptionally wire a use-case token.\n`,
  );

  // Collect available tokens from di.ts
  const availableTokens = gen.getActiveModules().flatMap((m) => {
    const cap = m.charAt(0).toUpperCase() + m.slice(1);
    return [`Create${cap}UseCaseToken`];
  });

  const opts = [
    {
      name: "None / manual",
      description: "Add a TODO stub instead",
      value: "__none__",
    },
    ...availableTokens.map((t) => ({
      name: t,
      description: "Wire from DI scope",
      value: t,
    })),
  ];
  ...
  const sel = new SelectRenderable(renderer, {
    ...
    focusedTextColor: "#93c5fd",
    selectedBackgroundColor: "#1d4ed8",
    height: Math.min(opts.length + 1, 10),
  });
  leftPane.add(sel);
  leftPane.add(
    new TextRenderable(renderer, { content: "\n[Esc] back", fg: "#6b7280" }),
  ...
  const cleanup = onEscape(() => showAddProcStep4(state));

  sel.on(
    SelectRenderableEvents.ITEM_SELECTED,
    (_idx: number, opt: { value: string }) => {
      cleanup();
      state.useCaseToken = opt.value === "__none__" ? undefined : opt.value;
      showAddProcConfirm(state);
    },
  );
};

// Step 6 ── Confirm
const showAddProcConfirm = (state: ProcedureConfig) => {
  clearLeft();
  setLeftTitle("Add Procedure · Confirm");
  clearLog(
    "━━━ Procedure Summary ━━━━━━━━━━━━\n" +
      `  Module    : ${state.moduleName}\n` +
      ...
      `  Access    : ${state.procedureAccess}Procedure\n` +
      `  UseCase   : ${state.useCaseToken ?? "none (TODO stub)"}\n\n` +
      `Will patch:\n  packages/api/src/routers/${state.moduleName}.router.ts\n` +
      "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n",
  );

  const opts = [
    { name: "✓ Add it", description: "Patch the router now", value: "go" },
    { name: "← Back", description: "Back to use-case step", value: "back" },
    { name: "✕ Cancel", description: "Return to main menu", value: "cancel" },
  ];

  leftPane.add(
    new TextRenderable(renderer, {
      content: "Ready to add procedure:\n",
      fg: "#10b981",
    }),
  );
  const sel = new SelectRenderable(renderer, {
  ...
  const cleanup = onEscape(() => showAddProcStep5(state));

  sel.on(
    SelectRenderableEvents.ITEM_SELECTED,
    (_idx: number, opt: { value: string }) => {
      cleanup();
      if (opt.value === "back") return showAddProcStep5(state);
      if (opt.value === "cancel") return showMainMenu();

      clearLog("Adding procedure...\n");
      const log: string[] = [];
      try {
        gen.addProcedureToRouter(state, log);
        appendLog(`${log.join("\n")}\n\n[✓] Procedure added.\n`);
      } catch (err) {
        appendLog(`\n[ERR] ${String(err)}\n`);
      }
