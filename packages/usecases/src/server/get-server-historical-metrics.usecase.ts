...
export class GetServerHistoricalMetricsUseCase {
  ...
  async execute(input: GetServerHistoricalMetricsInput): Promise<unknown> {
    ...
    const settings = await this.uow.monitoringSettingsRepository.findByServerId(serverId);
    if (!settings) {
      throw new Error(`Monitoring settings not configured for server ${serverId}`);
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Agent metrics response failed (${response.status}): ${text}`);
      }

      return await response.json();
    } catch (err) {
      throw new Error(`Failed to contact monitoring agent on ${serverIp}: ${err instanceof Error ? err.message : String(err)}`);
    }
  }
}
